...
object TankIndicatorTESR extends TileEntitySpecialRenderer[TileTankIndicator] {
  ...
  override def renderTileEntityAt(te: TileTankIndicator, x: Double, y: Double, z: Double, partialTicks: Float, destroyStage: Int): Unit = {
    ...
    if (!te.hasWorldObj || !BlockTankIndicator.Position.faces.exists(f => BlockTankIndicator.shouldSideBeRendered(te.getWorld, te.getPos.offset(f), f))) return
    for {
      core <- te.getCore
      ...
      val quads = for {
        face <- BlockTankIndicator.Position.faces if BlockTankIndicator.shouldSideBeRendered(te.getWorld, te.getPos.offset(face), face)
