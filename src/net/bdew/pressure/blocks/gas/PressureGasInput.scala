...
object BlockPressureGasInput extends BasePoweredBlock("gas_input", classOf[TilePressureGasInput]) with BlockNotifyUpdates with IPressureConnectableBlock {
...
class TilePressureGasInput extends TileDataSlotsTicking with IPressureInject with TileFilterable with GasHandlerProxy {
  ...
  serverTick.listen(() => {
    if (BlockInput.getSignal(world, pos)) {
      GasSupport.getGasHandler(world, pos.offset(getFacing.getOpposite), getFacing) map { from =>
