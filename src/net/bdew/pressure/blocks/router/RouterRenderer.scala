...
object RouterRenderer extends BaseBlockRenderHandler {
  ...
  override def renderWorldBlock(world: IBlockAccess, x: Int, y: Int, z: Int, block: Block, modelId: Int, renderer: RenderBlocks): Boolean = {
    ...
    for {
      tile <- Misc.asInstanceOpt(world.getTileEntity(x, y, z), classOf[TileRouter])
      ...
      ConnectedHelper.draw(face, 8).doDraw(Vec3F(x, y, z), RouterIcons.overlays(tile.sideModes.get(face)))
    }
    true
  }
}
